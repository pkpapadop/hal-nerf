...
from pose_regressor.script.utils import plot_features, save_image_saliancy
...
def eval_on_batch(args, data, model, feat_model, pose, img_idx, hwf, half_res, device, nerfacto_model, nerfacto_params):
    ...
    with torch.no_grad():
        ...
        target = data.numpy() # convert to numpy image
        target = torch.tensor(target)

        target = target.to(device)
        pose = pose.to(device)
        img_idx = img_idx.to(device)
        pose_nerf = pose_nerf.to('cpu')
...
def eval_on_epoch(args, data_loaders, model, feat_model, hwf, half_res, device, nerfacto_model, nerfacto_params):
    ...
    for data, pose, img_idx in val_dl:
        ...
        loss, psnr = eval_on_batch(args, data, model, feat_model, pose, img_idx, hwf, half_res, device, nerfacto_model, nerfacto_params)
...
def train_on_epoch(args, data_loaders, model, feat_model, hwf, optimizer, half_res, device, nerfacto_model, nerfacto_params):
    ''' Perform 1 epoch of training with batch '''
    model.train()
    ...
    for data, pose, img_idx in train_dl:
        ...
        loss, psnr = train_on_batch(args, data, model, feat_model, pose, img_idx, hwf, optimizer, half_res, device, nerfacto_model, nerfacto_params)
...
def train_feature_matching(args, model, feat_model, optimizer, i_split, hwf, nerfacto_model, nerfacto_params, device, early_stopping, images=None, poses_train=None, train_dl=None, val_dl=None, test_dl=None):
    ...
    for epoch in tqdm(range(N_epoch), desc='epochs'):
        ...
        loss, psnr = train_on_epoch(args, data_loaders, model, feat_model, hwf, optimizer, half_res, device, nerfacto_model, nerfacto_params)
