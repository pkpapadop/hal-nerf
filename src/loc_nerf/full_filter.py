...
from nerfacto_loader import upload_files
...
class NeRF:
    ...
    def __init__(self, nerf_params):
        ...
        if self.dataset_type == 'custom':
            ...
            _, _, self.W, self.H, _ = input_dataset(transform_path, self.obs_img_num, self.factor)
            transform = transforms.Compose([
                transforms.ToTensor(),
            ])
            # print("The filename is :")
            # print(self.image_filename)
            #print(self.obs_img_all[1].shape, self.obs_img_all[1])
            self.obs_img_pose = (self.obs_img_pose_all[self.obs_img_num])
            self.obs_img_pose = self.obs_img_pose["c2w"]
            self.obs_img = transform(self.obs_img_all[self.obs_img_num])
            #print( self.obs_img_pose)
            #img_dfnet = self.obs_img.permute(2, 0, 1)  # Reorder to [c, h, w]
            # img_dfnet = img_dfnet.unsqueeze(0)
            img_dfnet = self.obs_img
            img_dfnet = img_dfnet.unsqueeze(0).to('cuda')
            #self.obs_img = self.obs_img.permute(1, 2, 0)
            ...
            #print(self.obs_img.unsqueeze(0).shape)
            ...
            #self.obs_img_pose = {key: value.cpu().numpy() for key, value in self.obs_img_pose.items()}
    ...
    # Combine the translation and rotation into a single vector
            ...
            self.obs_img_noised = self.obs_img.permute(1, 2, 0)
        ...
        print(self.obs_img.shape, self.obs_img_pose, self.predicted_pose)
