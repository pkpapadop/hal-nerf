...
class NavigatorBase:
    def __init__(self, img_num=0, dataset_name=None):
        # AR start
        self.position_error_threshold = rospy.get_param('position_error_threshold')
        self.rotation_error_threshold = rospy.get_param('rotation_error_threshold')
        #self.std_threshold = rospy.get_param('std_threshold')
        self.termination_mode = rospy.get_param('termination_mode')
        self.particles_random_initial_position = rospy.get_param('particles_random_initial_position')
        self.particles_random_initial_rotation = rospy.get_param('particles_random_initial_rotation')
        self.output_path = rospy.get_param('output_path')
        self.export_images = rospy.get_param('export_images')
        ...
        path = Path(self.output_path + "/images")
        path.mkdir(parents=True, exist_ok=True)
        with open(self.output_path + '/results.txt', 'w') as file:
            file.write(f"num updates,num particles,std error,pos error,rot error\n")
        # AR end
        ...
        nerf_params = {'near':self.near, 'far':self.far, 'course_samples':self.course_samples, 'fine_samples':self.fine_samples,
                       ...
                       'netchunk':self.netchunk, 'chunk':self.chunk, 'bd_factor':self.bd_factor, 'export_images':self.export_images, 'output_path':self.output_path}
