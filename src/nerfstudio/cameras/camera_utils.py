...
def rotation_matrix(a: Float[Tensor, "3"], b: Float[Tensor, "3"]) -> Float[Tensor, "3 3"]:
    ...
    a = a.to('cuda')
    b = b.to('cuda')
    v = torch.cross(a, b).to('cuda')
    c = torch.dot(a, b).to('cuda')
    # If vectors are exactly opposite, we add a little noise to one of them
    if c < -1 + 1e-8:
        eps = (torch.rand(3) - 0.5) * 0.01
        return rotation_matrix(a + eps, b)
    s = torch.linalg.norm(v).to('cuda')
    ...
    eye = torch.eye(3).to('cuda')
    skew_sym_mat = skew_sym_mat.to('cuda')
    return eye + skew_sym_mat + skew_sym_mat @ skew_sym_mat * ((1 - c) / (s**2 + 1e-8))
