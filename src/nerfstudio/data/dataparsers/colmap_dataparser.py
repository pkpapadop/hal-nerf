...
class ColmapDataParser(DataParser):
    ...
    def _generate_dataparser_outputs(self, split: str = "train", **kwargs):
        ...
        if "applied_transform" in meta:
            applied_transform = torch.tensor(meta["applied_transform"], dtype=transform_matrix.dtype, device = "cuda")
            transform_matrix = transform_matrix @ torch.cat(
                [applied_transform, torch.tensor([[0, 0, 0, 1]], dtype=transform_matrix.dtype, device = 'cuda')], 0
            )
    ...
    def _load_3D_points(self, colmap_path: Path, transform_matrix: torch.Tensor, scale_factor: float):
        ...
        points3D = (
            torch.cat(
                (
                    points3D.cuda(),
                    torch.ones_like(points3D[..., :1].cuda()),
                ),
                -1,
            )
            @ transform_matrix.T
