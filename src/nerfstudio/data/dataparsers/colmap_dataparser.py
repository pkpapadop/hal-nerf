...
class ColmapDataParser(DataParser):
    ...
    def _generate_dataparser_outputs(self, split: str = "train", **kwargs):
        ...
        transform_matrix = transform_matrix.to('cuda')
        if "applied_transform" in meta:
            applied_transform = torch.tensor(meta["applied_transform"], dtype=transform_matrix.dtype).to('cuda')
            transform_matrix = transform_matrix @ torch.cat(
                [applied_transform, torch.tensor([[0, 0, 0, 1]], dtype=transform_matrix.dtype).to('cuda')], 0
            )
