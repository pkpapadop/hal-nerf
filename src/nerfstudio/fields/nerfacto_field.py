...
class NerfactoField(Field):
    ...
    def get_outputs(
        ...
        self.appearance_embedding_dim = torch.as_tensor(self.appearance_embedding_dim).clone().detach().to('cuda')
        self.embedding_apperanace = self.embedding_appearance.to('cuda')
        # appearance
        if self.training:
